...
{
  public class PostgresFilterGeneratorTest
  {
    [Fact]
    public void General()
    {
      ...
      var generator = new PostgresFilterGenerator(filter);
      ...
      Assert.Contains(dbFilter.Parameters,
        p => p.ParameterName == "@p7" && (DateTime) p.Value == DateTime.Today.AddDays(7));
    }
    ...
    public void Map()
    {
      ...
      var generator = new PostgresFilterGenerator(filter, (pn) => pn + "x");
      var dbFilter = generator.Generate();
