...
namespace LogicMine.DataObject.Filter.Parse
{
    ...
    public class FilterTermParser<T> : FilterTermParser
    {
        ...
        private Member GetTypeMember(string fieldName)
        {
            ...
            if (!_typeMembers.ContainsKey(fieldName))
                throw new InvalidOperationException(
                    $"No public member on {Type} called {fieldName} - case insensitive search");
        ...
        private void EnsureValidOperator(FilterOperators op, Type fieldType)
        {
            ...
            else if (op == FilterOperators.StartsWith || op == FilterOperators.Contains ||
                     op == FilterOperators.EndsWith)
            {
